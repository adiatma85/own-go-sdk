...
type Interface interface {
	...
	BatchSend(ctx context.Context, tokens []string, data map[string]string) ([]string, error)
	BatchSendDryRun(ctx context.Context, tokens []string) ([]string, error)
	SendToDevice(ctx context.Context, deviceToken string, payload map[string]string) (string, error)
}
...
// Batch sending tok multiple token, return slice of invalid tokens
func (m *messaging) BatchSend(ctx context.Context, tokens []string, data map[string]string) ([]string, error) {
	...
	multicastResponse, err := m.firebase.SendMulticast(ctx, message)
	...
	// If continue, then some token is not valid
	if multicastResponse.FailureCount > 0 {
		...
		return invalidTokens, errors.New("partial error multicast")
	}
...
func (m *messaging) BatchSendDryRun(ctx context.Context, tokens []string) ([]string, error) {
	...
	if multicastResponse.FailureCount > 0 {
		...
		return invalidTokens, errors.New("partial error multicast dry run")
	}

	return invalidTokens, nil
}

// Function to send a message to a Device via device token
func (m *messaging) SendToDevice(ctx context.Context, deviceToken string, payload map[string]string) (string, error) {
	message := &firebase_messaging.Message{
		Data:  payload,
		Token: deviceToken,
	}

	// This guarante return an error if the device token is not sent properly (e.g device token is malformed, or even does not exist)
	// return the message ID if the message is sent successfully
	messageId, err := m.firebase.Send(ctx, message)
	if err != nil {
		return "", err
	}
