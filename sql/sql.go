...
func (s *sqlDB) initDB() {
	...
	s.leader = initCommand(db, s.instrument, s.log, s.cfg.UseInstrument, s.cfg.LogQuery)
	...
	if s.isFollowerEnabled() {
		...
		s.follower = initCommand(db, s.instrument, s.log, s.cfg.UseInstrument, s.cfg.LogQuery)
	} else {
		s.follower = s.leader
	}
}
...
func (s *sqlDB) connect(toLeader bool) (*sqlx.DB, error) {
	...
	if s.cfg.UseInstrument {
		s.instrument.RegisterDBStats(sqlxDB.DB, "mysql")
	}
