...
func initCommand(db *sqlx.DB, instr instrument.Interface, log log.Interface, useInstr, logQuery bool) Command {
	return &command{
		db:            db,
		log:           log,
		instrument:    instr,
...
func (c *command) QueryIn(ctx context.Context, name string, query string, args ...interface{}) (*sqlx.Rows, error) {
	...
	if c.useInstrument {
		timer := c.instrument.MySQLQueryTimer(name)
...
func (c *command) Prepare(ctx context.Context, name string, query string) (CommandStmt, error) {
	...
	return initStmt(ctx, name, stmt, c.instrument, c.useInstrument), nil
}
...
func (c *command) BeginTx(ctx context.Context, name string, opt TxOptions) (CommandTx, error) {
	...
	return initTx(ctx, name, tx, opts, c.log, c.instrument, c.useInstrument, c.logQuery), nil
}
